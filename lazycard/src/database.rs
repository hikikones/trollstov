...
impl Database {
    ...
    pub fn is_tags_empty(&self) -> bool {
        self.sqlite
            .query_single("SELECT NOT EXISTS (SELECT 1 FROM tags)", (), |row| {
                row.get(0)
            })
            .unwrap()
    }
...
#[cfg(debug_assertions)]
const EMOJI_TAB: &str = "👻 oijwqwu qwdiowhq  i hio h qiowhqwheqw👻👻 wwq \
